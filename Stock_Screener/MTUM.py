...
def MTUM3():
    ...
    data = pd.read_csv('Dashboard\Stock_Screener\SAVE\STOCKS_Daily.csv',index_col = 0,header=[0, 1])
    save = []
    df = pd.DataFrame()
    ...
    #Regular
    df.to_csv('Dashboard\Stock_Screener\SAVE\MTUM_MA.csv')
    print(df)
    #EMA
    df3 = pd.concat(save)
    ...
    df3T.to_csv('Dashboard\Stock_Screener\SAVE\MTUM_EMA.csv')
    print(df3T)
